          ...
          <main className="main-page-nostr-container">
            ...
            >
              ...
              >
                ...
                <div
                  className="fit-container fx-centered fx-start-v feed-container"
                  style={{
                    overflow: "scroll",
                    marginBottom: "4rem",
...
const ExploreFeed = ({
  ...
  useEffect(() => {
    const initSub = async () => {
      setIsLoading(true);
      let dateCheckerArts = lastEventsTimestamps
        ? lastEventsTimestamps - 86400
        : Math.floor(Date.now() / 1000) - 86400;
      const artsFilter = getFilter();
      let articles = await getSubData(artsFilter);
      ...
      setContent((prev) =>
        removeEventsDuplicants([
          ...prev,
          ...articles_.map((event) => getParsedRepEvent(event)),
        ])
      );
      saveUsers(articles.pubkeys);
