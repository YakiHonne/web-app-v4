...
export default function SWCard({ widget, onNextWidget }) {
  ...
  const initZapping = async (recipientLNURL) => {
    try {
      ...
      setTriggerZap(data.data.callback);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <>
      ...
      <Widget
        ...
        event={widget}
        onZapButton={initZapping}
        onNostrButton={(addr) => customHistory.push(`/${addr}`)}
