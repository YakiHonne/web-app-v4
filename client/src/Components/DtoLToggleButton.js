...
export default function DtoLToggleButton({ isMobile = false, small = false }) {
  ...
  const { r, transform, cx, cy, opacity } =
    isDarkMode === "1" ? properties["moon"] : properties["sun"];
  ...
  // if (isMobile)
    return (
      <div className="fx-centered fx-start-h fit-container pointer"   onClick={setTheme}>
        <animated.svg
        ...
        <p className={isMobile ? "p-big link-label" : "link-label"}>Change theme</p>
      </div>
    );
  // return (
  //   <animated.svg
